...
const index = ({whichPage, idf}) => {
 ...
 const handleRowDeletion = (id) => {
  ...
  useEffect(() => {
    ...
    let roundnetamt = 0;
    var i = 0;
    for (const obj of Object.values(userContext.store.recdic.grid)) {
  ...
  const rowdatahandler = (myHandlerObj, val, data, eventType) => {
    if(data){
      ...
      console.log("purchase component",  userContext.store.recdic.pan);
        ...
        <div className="col-2">
          <div className="gst-col">
            <div>
              ...
              <span id="ttaxamt" className="value">{userContextPanstoreRender('ttaxamt')}</span>
            </div>
            <div>
              <span className="label">CGST</span>
              <span id="cgst" className="value">{userContextPanstoreRender('cgst')}</span>
            </div>
            <div>
              <span className="label">SGST</span>
              <span id="sgst" className="value">{userContextPanstoreRender('sgst')}</span>
            </div>
          </div>
          <div className="total-col">
            <div className="bill-total">
              ...
              <span id="gtot" className="value">
                ...
                {userContextPanstoreRender('gtot')}
              </span>
            </div>
            <div className="sub-total">
              <span  className="label">Sub Total</span>
              <span id="tsubtot" className="value">{userContextPanstoreRender('tsubtot')}</span>
            </div>
            <div className="roundoff">
              <span className="label">Round Off</span>
              <span id="roundoff" className="value">{userContextPanstoreRender('roundoff')}</span>
            </div>
            <div className="discount">
              <span className="label">Discount</span>
              <span id="tdisamt" className="value">{userContextPanstoreRender('tdisamt')}</span>
            </div>
            <div className="amt-total">
              <span className="label">Amount Total</span>
              <span id="tamt" className="value">{userContextPanstoreRender('tamt')}</span>
            </div>
          </div>
        </div>
      </div>
